...
(async()=>{
 ...
 CREATE TABLE public.credit_reservations (
  ...
  team_allocation_id uuid, feature_slug text, credits_reserved integer DEFAULT 0, trial_check_reserved boolean DEFAULT false,
 ...
 CREATE TABLE public.server_guest_sessions (
  ...
  trial_checks_total integer DEFAULT 1, total_used integer DEFAULT 0,
 ...
 CREATE TABLE public.usage_ledger (
  ...
  reservation_id uuid, team_owner_id uuid, error_reason text, ledger_type text,
 ...
 for(const file of ['00142_billing_expiry_and_metering.sql','00143_verified_payment_grants.sql','00144_plagiarism_billing_rate.sql']) await db.exec(fs.readFileSync(path.join(root,'supabase/migrations',file),'utf8'));
 ...
 await test('plagiarism charges 2 credits for 1-1000 words and 4 for 1001-2000',async()=>{
   await db.exec(`UPDATE profiles SET subscription_plan='pro',subscription_status='active',plan_end_date=now()+interval '30 days',credits_balance=20 WHERE id='${user}'`);
   const one=await reserve('plagiarism_check',1000,'plag-1000');assert.equal(one.allowed,true);assert.equal(Number(one.credits_reserved),2);
   const two=await reserve('plagiarism_check',1001,'plag-1001');assert.equal(two.allowed,true);assert.equal(Number(two.credits_reserved),4);
 });
 await test('failed plagiarism reservation refunds credits exactly once',async()=>{
   const before=Number((await query(`SELECT credits_balance FROM profiles WHERE id=$1`,[user]))[0].credits_balance);
   const r=await reserve('plagiarism_check',500,'plag-refund');assert.equal(r.allowed,true);assert.equal(Number(r.credits_reserved),2);
   await query(`SELECT finalize_credit_reservation($1,'failed')`,[r.reservation_id]);
   assert.equal(Number((await query(`SELECT credits_balance FROM profiles WHERE id=$1`,[user]))[0].credits_balance),before);
   assert.equal((await query(`SELECT finalize_credit_reservation($1,'failed') AS done`,[r.reservation_id]))[0].done,false);
 });
 await test('duplicate plagiarism submission cannot charge twice',async()=>{
   const first=await reserve('plagiarism_check',500,'plag-duplicate');assert.equal(first.allowed,true);
   const afterFirst=Number((await query(`SELECT credits_balance FROM profiles WHERE id=$1`,[user]))[0].credits_balance);
   assert.equal((await reserve('plagiarism_check',500,'plag-duplicate')).allowed,false);
   assert.equal(Number((await query(`SELECT credits_balance FROM profiles WHERE id=$1`,[user]))[0].credits_balance),afterFirst);
 });
