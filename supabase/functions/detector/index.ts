...
serve(async (req) => {
  ...
  try {
    ...
    const { text, sentence_level = true, paragraph_level = true, contentType, content_type, language, languageHint } = body;
    ...
    const analysisResult = await analyzeAdvancedText(text, {
      contentType: (contentType || content_type || 'auto') as any,
      detectorVersion: DETECTOR_VERSION,
      confidenceThreshold: 0.5,
      language: language || languageHint || 'auto',
      languageHint: languageHint || language,
      sentenceLevel: sentence_level,
      paragraphLevel: paragraph_level,
    });
