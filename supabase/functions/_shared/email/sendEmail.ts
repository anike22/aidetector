...
import { shouldSendEmail } from "../notifications.ts";
...
export interface SendEmailOptions {
  ...
  userId?: string;
  metadata?: any;
  category?: 'transactional' | 'security' | 'billing' | 'marketing' | 'notification' | 'invitation';
}

export async function sendEmail({ supabase, recipient, subject, html, text, type, template_name, userId, metadata, category = 'transactional' }: SendEmailOptions) {
  // Honor communication preferences for non-mandatory categories
  const emailAllowed = await shouldSendEmail(supabase, userId, category);
  if (!emailAllowed) {
    await supabase.from('email_logs').insert([{
      ...
      status: 'skipped',
      provider: 'resend',
      metadata: { ...(metadata || {}), skipped_reason: 'email_disabled' }
    }]);
