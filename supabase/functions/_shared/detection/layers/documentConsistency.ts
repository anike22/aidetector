...
export function analyzeDocumentConsistency(
  ...
  // A document that is all-AI or all-human does not qualify even if one paragraph
  // deviates — that is likely noise or a quoted passage, not mixed authorship.
  const hasStructuralMixedEvidence = aiLikeSections >= 2 && humanLikeSections >= 2;
