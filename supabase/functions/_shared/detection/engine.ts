...
import { analyzeLanguage, getLanguageName, isLanguageSupported } from './languageLayer.ts';
import { analyzeLinguistic, splitSentences, splitParagraphs, tokenizeWords } from './linguisticLayer.ts';
import { analyzeStatistical } from './statisticalLayer.ts';
import { runEnsemble } from './ensemble.ts';
import { getLanguageCalibration, languageReliabilityFactor } from './calibration.ts';
import { classifyWithClassifier, CLASSIFIER_VERSION } from './classifier.ts';
import { preprocessDocument } from './layers/documentPreprocessing.ts';
import { segmentPassages, type Passage } from './layers/passageSegmentation.ts';
import type { PassageScore } from './layers/humanEditDetection.ts';
import { analyzeSemanticConsistency } from './layers/semanticConsistency.ts';
...
function inferContentType(text: string, manual: ContentType): ContentType {
  ...
  if (
    (lower.includes('@') && (lower.includes('dear') || lower.includes('subject:') || lower.includes('from:'))) ||
    /(?:^|\n)\s*(?:sincerely|best regards|warm regards|kind regards|with regards)[,\s]*$/im.test(trimmed) ||
    /^(?:dear|hi|hello)\s+[a-z]+[,\s]/i.test(trimmed)
  ) return 'email';
...
function classifyLocalVerdict(
  ...
  if (Math.abs(ai - human) < 8 && max < 50) return 'mixed';
  ...
  const devMode = (() => {
    try {
      ...
      if (denoGlobal && typeof denoGlobal?.env?.get === 'function') {
        return denoGlobal.env.get('DETECTOR_DEV_MODE') === 'true';
      }
    } catch {
