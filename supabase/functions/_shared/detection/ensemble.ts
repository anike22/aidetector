...
const CONTENT_TYPE_CONFIG: Record<ContentType, ContentTypeWeights> = {
  auto:      { aiSensitivity: 1.00, humanTolerance: 1.10, formalPenalty: 0 },
...
export function runEnsemble(
  ...
  if (isSpanish) {
    ...
    const rawWeights = CONTENT_TYPE_CONFIG[contentType] ?? CONTENT_TYPE_CONFIG.auto;
    ...
    // -----------------------------------------------------------------
    const diversityBaseline = wordCount > 150
      ? Math.max(0.24, calibration.lexicalDiversityBaseline - 0.07 * Math.log(wordCount / 100))
      : calibration.lexicalDiversityBaseline;

    const repCeiling = wordCount > 150
      ? Math.min(0.85, 0.35 + 0.16 * Math.log(wordCount / 50))
      : 0.35;
    ...
    const aiFeatures = [
      ...
      Math.max(0, (diversityBaseline - linguistic.lexicalDiversity) / Math.max(0.2, diversityBaseline)) * 0.8 * weights.aiSensitivity,
    ...
    // -----------------------------------------------------------------
    const humanFeatures = [
      Math.min(1, Math.max(0, (linguistic.lexicalDiversity - diversityBaseline) * 3.5)) * 0.8 * weights.humanTolerance,
    ...
    const statAiFeatures = [
      ...
      normalize(statistical.repetitionDensity, 0, repCeiling) * 0.6,
    ...
    const statHumanFeatures = [
      ...
      Math.max(0, 1 - normalize(statistical.repetitionDensity, 0, repCeiling)) * 0.4,
    ...
    const independentAiSignals = [
      ...
      normalize(statistical.repetitionDensity, 0, repCeiling) > 0.35 ? 1 : 0,
  ...
  if (classifierInput && classifierInput.aiProbability >= 0 && classifierInput.aiProbability <= 1) {
    ...
    let weight = baseWeight * shortTextFactor * reliability;

    // Topic-bias discount: when character n-gram classifier flags AI (e.g. text discussing AI tools)
    // but the text displays unambiguous human linguistic structure (high sentence variance,
    // high burstiness, high stylistic variation, and zero boilerplate/formulaic markers),
    // discount the classifier weight to prevent topic keywords from overwhelming authentic human syntax.
    const isTopicBiasedHumanCandidate =
      classifierAiProbability > 0.60 &&
      wordCount >= 200 &&
      linguistic.sentenceLengthVariance >= 75 &&
      statistical.burstiness >= 0.65 &&
      linguistic.stylisticVariationScore >= 0.70 &&
      linguistic.aiBoilerplateScore <= 0.10 &&
      (linguistic.formulaicStartScore + linguistic.formulaicEndScore) === 0;

    if (isTopicBiasedHumanCandidate) {
      weight *= 0.25;
    }
...
function detectHumanizationSignals(
  ...
  // In long texts, lexical diversity naturally drops (Zipf's law), so only flag when diversity is exceptionally low
  // and personal voice markers are absent.
  if (statistical.sentenceVariance > 0.6 && linguistic.lexicalDiversity < 0.22 && linguistic.personalVoiceScore < 0.15) {
  ...
  if (linguistic.aiBoilerplateScore > 0.5 && linguistic.stylisticVariationScore > 0.6 && linguistic.personalVoiceScore < 0.15) {
