...
BEGIN
  ...
  -- Insert with the recursive trigger disabled for this dedicated test row only.
  ALTER TABLE public.profiles DISABLE TRIGGER trg_profiles_customer_profile;
  INSERT INTO profiles (id, email, full_name, subscription_plan, trial_checks_remaining, trial_checks_used, created_at, updated_at)
  VALUES ('11111111-1111-4111-8111-111111111111', 'billing-test-3@aicx-test.local', 'Billing Test 3', 'free', 5, 0, now(), now());
  ALTER TABLE public.profiles ENABLE TRIGGER trg_profiles_customer_profile;
  ...
  FOR v_i IN 1..4 LOOP
    SELECT * INTO v_res FROM reserve_entitlement_and_credits(
      p_user_id := '11111111-1111-4111-8111-8111-111111111111'::uuid, p_feature_slug := 'ai_detector', p_credits_cost := 1, p_timezone := 'UTC'
    );
