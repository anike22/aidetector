...
BEGIN
  ...
  SELECT subscription_plan, subscription_status, credits_balance, plan_end_date
  INTO v_orig_plan, v_orig_status, v_orig_credits, v_orig_end
  FROM profiles WHERE id = v_uid;

  UPDATE profiles
  SET subscription_plan = 'pro', subscription_status = 'active', credits_balance = 100, plan_end_date = now() + interval '30 days'
  WHERE id = v_uid;
  ...
  UPDATE profiles
  SET subscription_plan = v_orig_plan, subscription_status = v_orig_status, credits_balance = v_orig_credits, plan_end_date = v_orig_end
  WHERE id = v_uid;
END $$;