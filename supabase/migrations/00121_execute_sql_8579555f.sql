...
BEGIN
  ...
  -- handle_new_user auto-creates the profile; align its trial fields.
  ...
  IF v_profile.trial_checks_remaining IS NULL OR v_profile.trial_checks_remaining <> 5 THEN
    RAISE EXCEPTION 'TEST 3 SETUP FAIL: profile not auto-created with 5 checks (got %)', v_profile.trial_checks_remaining; END IF;
