-- =====================================================================
-- Guest session hardening v3: throttle ALL new-session creation per IP
--
-- Hole: a client-supplied brand-new guest id (x-guest-id) previously
-- created a session WITHOUT the per-IP throttle (the throttle only ran
-- on the server-generated path). Rotating client ids → unlimited fresh
-- trial checks.
--
-- Fix: any session CREATION (row does not exist yet) counts toward the
-- per-IP daily cap, whether the id came from the client or the server.
-- Returning visitors (existing row) are untouched — shared networks
-- with existing sessions are not disqualified.
-- =====================================================================
