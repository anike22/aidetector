-- 00108_harmonize_reservations_and_finalization.sql
...
BEGIN
  ...
  IF FOUND THEN
    v_effective_cost := COALESCE(v_rate.base_credit_cost, p_credits_cost::INTEGER);
    v_is_trial_eligible := COALESCE(v_rate.trial_eligible, false);
  ELSE
    v_effective_cost := p_credits_cost::INTEGER;
...
BEGIN
  ...
  v_cost := COALESCE(v_res.credits_reserved, 0);
  v_is_trial := (v_res.reservation_type = 'trial_check');
