...
DECLARE
  ...
  v_orig_trial int;
BEGIN
  -- Set role to service_role so p_user_id is respected
  PERFORM set_config('role', 'service_role', true);

  SELECT subscription_plan, subscription_status, credits_balance, trial_checks_remaining
  INTO v_orig_plan, v_orig_status, v_orig_bal, v_orig_trial
  FROM profiles WHERE id = v_test_uid;

  -- Setup paid subscriber with trial_checks_remaining = 0
  UPDATE profiles SET subscription_plan='pro', subscription_status='active',
    credits_balance=100, trial_checks_remaining=0,
  ...
  -- Clean up test records and restore original user state
  DELETE FROM credit_reservations WHERE user_id = v_test_uid;
  DELETE FROM usage_ledger WHERE user_id = v_test_uid;
  UPDATE profiles SET subscription_plan = v_orig_plan, subscription_status = v_orig_status,
    credits_balance = v_orig_bal, trial_checks_remaining = v_orig_trial
  WHERE id = v_test_uid;
END $$;