...
BEGIN
  ...
  IF FOUND THEN
    ...
    -- If caller explicitly passed a higher credits cost (e.g. fixed 30 credits for blogger/SEO optimization workflow), honor it
    IF p_credits_cost IS NOT NULL AND p_credits_cost > 0 THEN
      v_effective_cost := GREATEST(v_effective_cost, p_credits_cost::integer);
    END IF;
  ...
  IF FOUND THEN
    ...
    IF v_team_alloc.status = 'active' AND billing_paid_active(v_owner.subscription_plan, v_owner.subscription_status, v_owner.plan_end_date)
      ...
      RETURN QUERY SELECT
        TRUE, v_res_id,
        v_owner.subscription_plan::TEXT,
        (v_owner.credits_balance)::NUMERIC,
        v_effective_cost::NUMERIC,
        (v_team_alloc.allocated_credits - v_team_alloc.consumed_credits)::NUMERIC,
        v_team_alloc.allocated_credits::NUMERIC,
        FALSE, 0::INTEGER, 0::INTEGER,
        'Team credits reserved successfully.'::TEXT, NULL::TEXT;
      RETURN;
    END IF;
  END IF;
  ...
  RETURN QUERY SELECT
    ...
    FALSE, COALESCE(v_profile.trial_checks_remaining, 0)::INTEGER, v_intro_total,
    'Insufficient credits. Required: ' || v_effective_cost || ' credits. Current balance: ' || COALESCE(v_profile.credits_balance, 0) || ' credits.'::TEXT,
