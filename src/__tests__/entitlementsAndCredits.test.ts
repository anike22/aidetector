...
describe('Entitlements & Central Rate Table System', () => {
  ...
  describe('Central Rate Table Calculations', () => {
    ...
    it('calculates Humanizer rewrite cost (10 cr per started 1,000 input words)', () => {
      // 400 words => 10 credits
      expect(calculateOperationCreditCost('humanizer_rewrite', { words: 400 })).toBe(10);
      // 1,200 words => 20 credits
      expect(calculateOperationCreditCost('humanizer_rewrite', { words: 1200 })).toBe(20);
    });

    it('calculates Plagiarism search cost (10 cr per started 1,000 words)', () => {
      expect(calculateOperationCreditCost('plagiarism_check', { words: 800 })).toBe(10);
      expect(calculateOperationCreditCost('plagiarism_check', { words: 2100 })).toBe(30);
    });

    it('calculates SEO Assistant report cost (30 cr per comprehensive report)', () => {
      expect(calculateOperationCreditCost('seo_assistant', { words: 950 })).toBe(30);
    });

    it('calculates Image analysis cost (5 cr standard, 10 cr advanced deepfake)', () => {
      expect(calculateOperationCreditCost('image_detect_standard', { images: 1 })).toBe(5);
      expect(calculateOperationCreditCost('image_detect_standard', { images: 3 })).toBe(15);
      expect(calculateOperationCreditCost('image_detect_advanced', { images: 1 })).toBe(10);
      expect(calculateOperationCreditCost('deepfake_detector', { images: 1 })).toBe(10);
    });

    it('calculates Video analysis cost (15 cr/30s across modes)', () => {
      // 25s Balanced => 15 credits
      expect(calculateOperationCreditCost('video_detect_balanced', { videoSeconds: 25 })).toBe(15);
      // 60s Balanced => 30 credits (2 slots * 15)
      expect(calculateOperationCreditCost('video_detect_balanced', { videoSeconds: 60 })).toBe(30);
      // 45s High-Sensitivity => 30 credits (2 slots * 15)
      expect(calculateOperationCreditCost('video_detect_high_sensitivity', { videoSeconds: 45 })).toBe(30);
      // 30s Forensic => 15 credits
      expect(calculateOperationCreditCost('video_detect_forensic', { videoSeconds: 30 })).toBe(15);
    });

    it('calculates Voice analysis cost (5 cr per started minute)', () => {
      expect(calculateOperationCreditCost('voice_analysis', { audioMinutes: 0.5 })).toBe(5);
      expect(calculateOperationCreditCost('voice_analysis', { audioMinutes: 2.2 })).toBe(15);
    });

    it('calculates Citation verification cost (5 cr per 5 references)', () => {
      expect(calculateOperationCreditCost('citation_verify', { references: 4 })).toBe(5);
      expect(calculateOperationCreditCost('citation_verify', { references: 12 })).toBe(15);
    });
  });
