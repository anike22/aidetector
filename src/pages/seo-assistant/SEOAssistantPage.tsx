...
export default function SEOAssistantPage() {
  ...
  const { user } = useAuth();
  const { entitlement, loading: entitlementLoading, recordUsage } = useEntitlement(FEATURE_SLUG);
    ...
    <MainLayout showFooter={false}>
      ...
      <div className="border-b border-border bg-card px-4 md:px-6 py-3 flex flex-col md:flex-row md:items-center gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0 flex-wrap">
          <UsageBadge featureSlug={FEATURE_SLUG} label="Daily AI uses" unit="uses" />
          {content && (
