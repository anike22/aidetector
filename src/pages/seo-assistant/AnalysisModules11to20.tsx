...
import { Copy, Download, Check, Loader2, Globe, Sparkles, FileText, Award, ShieldCheck, ChevronLeft, ChevronRight, ChevronDown, X, Search, RefreshCw, Link2, ExternalLink, AlertCircle, CheckCircle2, Info } from 'lucide-react';
...
export function EEATPanel({ 
  ...
  const hookTemplates = [
    {
      type: 'experience',
      title: 'Personal Experience Hook',
      show: !result.hasPersonalExamples,
      target: 'intro' as const,
      text: 'In our practical tests and hands-on testing, we found that focusing on clear, verifiable examples consistently delivers the best results.',
    },
    {
      type: 'stats',
      title: 'Statistical / Research Hook',
      show: !result.hasStats,
      target: 'after_h1' as const,
      text: 'According to recent industry survey data and benchmarks, over 74% of high-ranking articles incorporate verifiable statistics and case studies.',
    },
    {
      type: 'citation',
      title: 'Authoritative Citation Anchor',
      show: !result.hasCitations,
      target: 'end' as const,
      text: '**References & Verified Sources:**\n- Official Guidelines & Industry Technical Report (https://example.com/reference)\n- Empirical Benchmark Analysis, 2024.',
    },
    {
      type: 'author',
      title: 'Author & Fact-Check Attribution',
      show: !result.hasAuthor,
      target: 'after_h1' as const,
      text: '> **Written by Editorial Team · Reviewed by Industry Specialist**\n> *Last updated and fact-checked for technical accuracy.*',
    },
  ];

  const activeHooks = hookTemplates.filter(h => h.show);
  ...
  return (
    <AnalysisModule title="EEAT Analysis" score={result.score}>
      ...
      {activeHooks.length > 0 && onInsertHook && (
        <div className="mb-2 p-2 bg-primary/5 border border-primary/20 rounded flex flex-col gap-1.5">
          <div className="text-[10px] font-semibold text-primary uppercase flex items-center gap-1">
            <Sparkles className="w-3 h-3 text-primary" /> Contextual EEAT Opportunities
          </div>
          <p className="text-[10px] text-muted-foreground">Insert pre-formatted EEAT anchors directly into your article:</p>
          <div className="flex flex-col gap-1 mt-0.5">
            {activeHooks.map((h, i) => (
              <div key={i} className="flex items-center justify-between gap-1 p-1.5 bg-background border border-border rounded">
                <span className="text-[10px] font-medium text-foreground truncate">{h.title}</span>
                <div className="flex items-center gap-1 shrink-0">
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => setSelectedHook(selectedHook?.title === h.title ? null : h)}
                    className="h-5 text-[9px] px-1.5 text-muted-foreground hover:text-foreground"
                  >
                    {selectedHook?.title === h.title ? 'Hide' : 'Preview'}
                  </Button>
                  <Button
                    size="sm"
                    onClick={() => {
                      onInsertHook(h.text, h.target);
                      toast.success(`Inserted ${h.title} into article!`);
                    }}
          ...
          {selectedHook && (
            <div className="p-2 bg-muted/40 border border-border rounded text-[10px] flex flex-col gap-1">
              <span className="font-semibold text-foreground">Preview ({selectedHook.target}):</span>
              <p className="text-muted-foreground font-mono italic whitespace-pre-wrap">{selectedHook.text}</p>
            </div>
          )}
        </div>
      )}
...
export function InternalLinkingPanel({
  ...
  const currentDomain = initialDomain || localDomain;
    ...
    >
      <div className="flex flex-col gap-2 mb-3">
        ...
        <div className="flex gap-2">
          <Input 
            placeholder="e.g. zerogpt.com or yoursite.com" 
            value={currentDomain} 
