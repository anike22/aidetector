...
export default function ApiDashboardPage() {
  ...
  const generateKey = async () => {
    ...
    if (error) { toast.error('Failed to generate key: ' + error.message); }
      ...
      setKeys([data, ...keys]); setNewKeyName(''); setShowCreate(false); toast.success('API key generated successfully!');
      trackLifecycleEvent('first_api_key', { key_name: data.name });
      trackLifecycleEvent('api_request', { action: 'generate_key' });
      trackBehaviorEvent({ event_type: 'api_key_generated', event_category: 'api', event_data: { key_name: data.name } });
    }
    setCreating(false);
  };
