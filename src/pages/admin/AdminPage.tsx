...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    else if (path.includes('/admin/settings')) setActiveSection('settings');
...
function MonetizationSection() {
  ...
  const [adUnits, setAdUnits] = useState([
    { id: 1, name: 'Sidebar Banner', slot: '1234567890', type: 'Display', active: true },
    { id: 2, name: 'In-Article Responsive', slot: '0987654321', type: 'In Article', active: false },
  ]);
  const [editingAd, setEditingAd] = useState<{ id: number | null; name: string; slot: string; type: string; active: boolean } | null>(null);
  const [editingTag, setEditingTag] = useState<{ id: number | null; provider: string; tag: string; enabled: boolean } | null>(null);
            ...
            <CardHeader className="pb-3 border-b border-border flex flex-row items-center justify-between">
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <Button size="sm" variant="outline" className="h-8" onClick={() => setEditingAd({ id: null, name: '', slot: '', type: 'Display', active: true })}>Add Ad Unit</Button>
                </DialogTrigger>
                <DialogContent className="max-w-[calc(100%-2rem)] md:max-w-lg">
                  <DialogHeader>
                    <DialogTitle>{editingAd?.id ? 'Edit Ad Unit' : 'Add Ad Unit'}</DialogTitle>
                    <DialogDescription>Manage AdSense ad placements for your site.</DialogDescription>
                  </DialogHeader>
                  <div className="space-y-4 py-2">
                    <div className="space-y-2">
                      <Label htmlFor="ad-name">Ad Name</Label>
                      <Input id="ad-name" value={editingAd?.name || ''} onChange={(e) => setEditingAd(a => a ? { ...a, name: e.target.value } : a)} />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="ad-slot">Ad Slot ID</Label>
                      <Input id="ad-slot" value={editingAd?.slot || ''} onChange={(e) => setEditingAd(a => a ? { ...a, slot: e.target.value } : a)} />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="ad-type">Ad Type</Label>
                      <Input id="ad-type" value={editingAd?.type || ''} onChange={(e) => setEditingAd(a => a ? { ...a, type: e.target.value } : a)} />
                    </div>
                    <div className="flex items-center gap-2">
                      <Switch id="ad-active" checked={editingAd?.active || false} onCheckedChange={(c) => setEditingAd(a => a ? { ...a, active: c } : a)} />
                      <Label htmlFor="ad-active">Active</Label>
                    </div>
                  </div>
                  <DialogFooter>
                    <Button onClick={() => {
                      if (!editingAd?.name || !editingAd?.slot) return;
                      if (editingAd.id) {
                        setAdUnits(prev => prev.map(u => u.id === editingAd.id ? { ...editingAd, id: u.id } : u));
                        toast.success('Ad unit updated');
                      } else {
                        setAdUnits(prev => [...prev, { ...editingAd, id: Date.now() }]);
                ...
                <tbody className="divide-y border-border">
                  {adUnits.map((unit) => (
                    <tr key={unit.id} className="hover:bg-muted/20">
                      <td className="px-5 py-3 font-medium text-navy">{unit.name}</td>
                      <td className="px-5 py-3 text-muted-foreground font-mono text-xs">{unit.slot}</td>
                      <td className="px-5 py-3 text-muted-foreground">{unit.type}</td>
                      <td className="px-5 py-3">
                        <Badge variant="outline" className={unit.active ? 'border-success text-success bg-success/10' : 'text-muted-foreground'}>
                      ...
                      <td className="px-5 py-3 text-right">
                        <div className="flex justify-end gap-2">
                          <Dialog>
                            <DialogTrigger asChild>
                              <Button variant="ghost" size="sm" className="h-7 px-2" onClick={() => setEditingAd({ ...unit })}>Edit</Button>
                            </DialogTrigger>
                            <DialogContent className="max-w-[calc(100%-2rem)] md:max-w-lg">
                              <DialogHeader>
                                <DialogTitle>Edit Ad Unit</DialogTitle>
                                <DialogDescription>Update this ad placement.</DialogDescription>
                              </DialogHeader>
                              <div className="space-y-4 py-2">
                                <div className="space-y-2">
                                  <Label htmlFor={`ad-name-${unit.id}`}>Ad Name</Label>
                                  <Input id={`ad-name-${unit.id}`} value={editingAd?.name || ''} onChange={(e) => setEditingAd(a => a ? { ...a, name: e.target.value } : a)} />
                                </div>
                                <div className="space-y-2">
                                  <Label htmlFor={`ad-slot-${unit.id}`}>Ad Slot ID</Label>
                                  <Input id={`ad-slot-${unit.id}`} value={editingAd?.slot || ''} onChange={(e) => setEditingAd(a => a ? { ...a, slot: e.target.value } : a)} />
                                </div>
                                <div className="space-y-2">
                                  <Label htmlFor={`ad-type-${unit.id}`}>Ad Type</Label>
                                  <Input id={`ad-type-${unit.id}`} value={editingAd?.type || ''} onChange={(e) => setEditingAd(a => a ? { ...a, type: e.target.value } : a)} />
                                </div>
                                <div className="flex items-center gap-2">
                                  <Switch id={`ad-active-${unit.id}`} checked={editingAd?.active || false} onCheckedChange={(c) => setEditingAd(a => a ? { ...a, active: c } : a)} />
                                  <Label htmlFor={`ad-active-${unit.id}`}>Active</Label>
                                </div>
                              </div>
                              <DialogFooter>
                                <Button onClick={() => {
                          ...
                          <Button variant="ghost" size="sm" className="h-7 px-2 text-destructive" onClick={() => {
                            setAdUnits(prev => prev.filter(u => u.id !== unit.id));
          ...
          <Card className="border-border shadow-sm">
            <CardHeader className="pb-3 border-b border-border flex flex-row items-center justify-between">
              ...
              <Button size="sm" variant="outline" className="h-8" onClick={() => {
                const id = metaTags.length > 0 ? Math.max(...metaTags.map(t => t.id)) + 1 : 1;
                setMetaTags([...metaTags, { id, provider: 'New Provider', tag: '', enabled: true }]);
                toast.success('New provider added — enter the tag and save');
              }}>Add Provider</Button>
            </CardHeader>
            <CardContent className="p-5 space-y-5">
