...
import { useGatedStreamLLM } from '@/hooks/useGatedStreamLLM';
...
const FEATURE_SLUG = 'seo_content_studio';
const FEATURE_NAME = 'SEO Content Studio';
...
export default function Step6ArticleGeneration({
 state, onChange, onNext, onBack }: Props) {
  const { open, featureName, trigger, remaining, limit, openUpgradeModal, closeUpgradeModal } = useUpgradeModal();
  const streamLLM = useGatedStreamLLM(FEATURE_SLUG, FEATURE_NAME, openUpgradeModal);
