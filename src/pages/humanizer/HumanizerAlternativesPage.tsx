...
import { RefreshCw, CheckCircle2, ChevronRight, Scale, BookOpen, Zap, AlertCircle, Loader2, ArrowLeft, RotateCcw, Play, Copy, Check, BookmarkCheck, History } from 'lucide-react';
...
export default function HumanizerAlternativesPage() {
  ...
  const handleCopy = async (type: string, text?: string) => {
    ...
    try {
      ...
      setCopiedType(type);
      toast.success(`Copied ${type} version to clipboard!`);
      setTimeout(() => {
  ...
  return (
    <div className="container mx-auto py-6 md:py-10 px-4 md:px-6 max-w-7xl">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6 md:mb-8 pb-4 border-b">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight mb-1">Select Best Version</h1>
          <p className="text-sm text-muted-foreground">
            We generated three distinct alternatives for your text. Copy any version directly or select your favorite.
          </p>
        </div>
        <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
          <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200 flex items-center gap-1.5 py-1.5 px-3">
            <BookmarkCheck className="h-4 w-4 text-emerald-600" />
            <span className="font-medium text-xs">All 3 versions auto-saved</span>
          </Badge>
          <Button
            variant="outline"
            size="sm"
            onClick={() => navigate('/humanizer/history')}
        ...
        {SLOTS.map(slot => {
          ...
          return (
            <Card key={slot.type} className={`flex flex-col h-full ${selecting === slot.type ? 'border-primary ring-2 ring-primary/20' : ''} ${isFailed ? 'border-destructive/30 bg-destructive/5' : ''}`}>
                    ...
                    <div className="p-3 md:p-4 flex-1">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          className="h-7 px-2.5 text-xs flex items-center gap-1.5 hover:bg-primary/10 hover:text-primary transition-colors border-dashed"
                          onClick={(e) => {
                            e.stopPropagation();
                            handleCopy(slot.type, alt.text);
                          }}
                          title="Copy this humanized text to clipboard"
                        >
                          {copiedType === slot.type ? (
                            <>
              ...
              <CardFooter className="p-3 md:p-4 border-t bg-muted/10 flex flex-col sm:flex-row gap-2">
                {isCompleted && alt.text ? (
                  <>
                    <Button
                      variant="outline"
                      ...
                      onClick={() => handleCopy(slot.type, alt.text)}
                      title="Copy complete text to clipboard"
                    >
                      {copiedType === slot.type ? (
                        <>
                    ...
                    <Button
                      className="w-full sm:w-auto flex-[1.4]"
                      size="default"
