...
export default function DeepfakeDetector() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const reservation = await reserveEntitlement('deepfake_detector', 10, { images: 1 });
      if (!reservation.allowed) {
        setScanning(false);
        openUpgradeModal({ featureName: 'Deepfake Detector', trigger: 'pro_feature' });
        toast.error(reservation.reason || 'Insufficient credits or plan required.');
        return;
      }
      ...
      if (reservationId) {
        await finalizeReservation(reservationId, 'committed', { featureSlug: 'deepfake_detector' });
      }
      setResult(r);
      ...
      if (reservationId) {
        await finalizeReservation(reservationId, 'released', { error: err.message });
      }
      toast.error(err.message || 'Deepfake analysis failed.');
    } finally {
      setScanning(false);
    }
  };
