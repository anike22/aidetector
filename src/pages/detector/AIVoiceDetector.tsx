...
export default function AIVoiceDetector() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const reservation = await reserveEntitlement('voice_analysis', 5, { audioMinutes: 1 });
      if (!reservation.allowed) {
        setIsAnalyzing(false);
        openUpgradeModal({ featureName: 'AI Voice / Audio Detector', trigger: 'pro_feature' });
      ...
      if (reservationId) {
        await finalizeReservation(reservationId, 'committed', { featureSlug: 'voice_analysis' });
      }
      setResult(res);
