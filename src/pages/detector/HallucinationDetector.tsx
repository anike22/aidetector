...
const FEATURE_SLUG = 'hallucination_detector';
...
export default function HallucinationDetector() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const reservation = await reserveEntitlement(FEATURE_SLUG, 5, { words: wordCount });
      if (!reservation.allowed) {
        setIsAnalyzing(false);
        openUpgradeModal({ featureName: 'Hallucination Detector', trigger: 'pro_feature' });
        toast.error(reservation.reason || 'This feature requires an active subscription or credits.');
        return;
      }
      ...
      if (reservationId) {
        await finalizeReservation(reservationId, 'committed', { featureSlug: FEATURE_SLUG });
      }
      setResult(res);
      toast.success('Hallucination analysis completed.');
    } catch (e: any) {
      if (reservationId) {
        await finalizeReservation(reservationId, 'released', { error: e.message });
      }
      toast.error(e.message || 'Hallucination check failed.');
    } finally {
      setIsAnalyzing(false);
    }
  };
