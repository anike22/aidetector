...
export async function simulateDetection(file: File, mode: MediaDetectionMode): Promise<DetectionResult> {
  // If file is an image, execute real evidence-based forensic analysis
  if (mode === 'image' || file.type.startsWith('image/')) {
    try {
      const real = await executeRealImageForensics(file);
      const aiScore = real.aiGeneration.score;
      const status: DetectionStatus =
        real.aiGeneration.verdict === 'Likely AI-generated'
          ? 'Likely AI-generated'
          : real.aiGeneration.verdict === 'AI editing indicated'
          ? 'Possibly edited'
          : real.aiGeneration.verdict === 'No strong AI-generation evidence'
          ? 'Authentic / No AI signs detected'
          : 'Inconclusive';

      const c2paStatus: C2PAStatus =
        real.provenance.c2paStatus === 'Valid trusted credentials'
          ? 'Verified'
          : real.provenance.c2paStatus === 'Invalid credentials'
          ? 'Broken'
          : real.provenance.c2paStatus === 'Absent credentials'
          ? 'Missing'
          : 'Not checked';

      return {
        aiProbability: aiScore,
        confidenceLevel: real.aiGeneration.confidenceLevel,
        status,
        riskLevel: riskFromProbability(aiScore),
        breakdown: {
          faceAuthenticity: 100 - real.aiGeneration.spectralArtifactScore,
          backgroundConsistency: 100 - real.manipulation.elaDeltaScore,
          lightingConsistency: 100 - real.aiGeneration.noiseUniformityScore,
          metadataCheck: real.provenance.manifestPresent ? 95 : 50,
          deepfakeRisk: real.aiGeneration.spectralArtifactScore,
          aiArtifactScore: real.aiGeneration.spectralArtifactScore,
          audioAuthenticity: 0,
          provenanceCheck: real.provenance.c2paStatus === 'Valid trusted credentials' ? 100 : 40,
        },
        detectedSignals: [
          ...real.aiGeneration.reasoning,
          ...real.manipulation.reasoning,
          ...real.provenance.reasoning,
        ].slice(0, 8),
        explanation: buildExplanation(aiScore, real.aiGeneration.reasoning, mode),
        c2pa: {
          status: c2paStatus,
          editingHistory: real.provenance.editHistory.join(', ') || null,
          aiDisclosure: real.provenance.claimGenerator ? `Detected generator: ${real.provenance.claimGenerator}` : null,
        },
        suspiciousTimestamps: [],
        recommendation: buildRecommendation(aiScore, status),
        fileName: file.name,
        fileType: file.type || 'image/jpeg',
        fileSize: file.size,
        scanDate: real.analyzedAt,
        mode,
      };
    } catch (err) {
      console.warn('Real forensic execution fell back for legacy caller:', err);
    }
  }

  const rng = seededRng(fileSeed(file));
  const baseProbability = 20 + Math.floor(rng() * 40);
  const aiProbability = Math.min(85, Math.max(10, baseProbability));
...
// --- Text Analysis (Advanced Multilingual Engine) ---
export type {
  AdvancedTextAnalysisResult as TextAnalysisResult,
  ContentType,
  Verdict,
  DetectedLanguage,
  LanguageSegment,
...
} from '@/lib/detection/detectorApi';

export { getLanguageName } from '@/lib/detection/languageLayer';

// --- Plagiarism Analysis ---

/**
 * Status returned with every plagiarism result.
 *
 * - completed            : full analysis done, ≥1 provider succeeded
 * - partial              : ≥1 provider ran; others failed
 ...
 * - analysis_failed      : unexpected server error
 *
 * IMPORTANT: no_verified_matches does NOT mean "definitely original."
 * It only means no match was found in the sources that were checked.
 */
...
/** One matched span between submitted text and a verified source. */
...
/** A verified source with evidence for every reported match. */
export interface VerifiedSource {
  ...
  /** Non-overlapping marginal percentage contributed by this source (0–100). */
...
/**
 * Analyze text for plagiarism against real scholarly sources.
 *
...
export async function analyzePlagiarism(text: string, language?: string): Promise<PlagiarismAnalysisResult> {
  ...
  // Server returned a structured error (auth / entitlement / validation)
  if (data.status === 'analysis_failed' || data.status === 'provider_unavailable') {
    const structured = data as PlagiarismAnalysisResult;
    // Preserve entitlement metadata so UI can show upgrade prompt
    return structured;
  }

  // Validate the result has a known status — reject any unexpected shape
  const knownStatuses: PlagiarismStatus[] = [
...
async function buildGuestFingerprint(): Promise<string> {
  try {
    const raw = [
      navigator.language,
      navigator.platform,
      ...
      new Date().toISOString().slice(0, 10), // date component for daily rotation
    ].join('|');
