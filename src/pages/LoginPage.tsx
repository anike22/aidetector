...
function safeReturnUrl(returnTo: string | null): string {
  if (!returnTo) return '';
  try {
...
export default function LoginPage() {
  ...
  const returnTo = useMemo(() => safeReturnUrl(searchParams.get('returnTo') || searchParams.get('return_to') || searchParams.get('redirect')), [searchParams]);
  ...
  const finalizeRedirect = (fallback = '/') => {
    const stored = sessionStorage.getItem('post_auth_redirect');
    if (stored) {
      sessionStorage.removeItem('post_auth_redirect');
      const safeStored = safeReturnUrl(stored);
      if (safeStored) {
        navigate(safeStored);
        return;
      }
  ...
  useEffect(() => {
    if (searchParams.get('verified') === 'true') {
      ...
      // Clean up verification params while preserving returnTo
      const next = new URLSearchParams();
      if (returnTo) next.set('returnTo', returnTo);
      setSearchParams(next, { replace: true });
    }
  }, [searchParams, returnTo, setSearchParams]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (authError) {
      ...
      trackLifecycleEvent('login', { source: 'email' });
      // Link guest session to the authenticated user in background
      try {
