...
const faqItems = [
  ...
  },
  {
    ...
    a: 'Our plagiarism checker compares submitted text against verified academic sources from Crossref, OpenAlex and Unpaywall. It requires actual matching wording (exact match) or strong lexical evidence (near match) before reporting a verified source. Semantic similarity may flag a candidate, but it does not count as confirmed plagiarism unless source text is retrieved and the evidence is defensible.',
  },
  {
    q: 'What is the difference between exact plagiarism and paraphrased plagiarism?',
    a: 'Exact plagiarism is a word-for-word copy of source material. Near or paraphrased plagiarism rewrites the original with limited changes while preserving the same wording and structure. Our checker only reports verified paraphrases when retrieved source passages provide defensible evidence; topic similarity alone is not treated as plagiarism.',
  },
  {
...
export default function PlagiarismCheckerPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setResult(res);
      // Only track when a real analysis completed (not a failure status)
      if (res.status !== 'analysis_failed' && res.status !== 'provider_unavailable') {
        ...
        trackBehaviorEvent({
          event_type: 'plagiarism',
          event_category: 'tool',
          event_data: { word_count: wordCount, similarity_score: res.similarityScore },
        });
      }
    } catch {
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
            ...
            <div className="lg:col-span-7 flex flex-col gap-4">
              ...
              {/* Error panel — network-level failures (thrown, not returned as result) */}
              {error && (
                <Card className="border-destructive/30 bg-destructive/5 shadow-card">
                  <CardContent className="p-4 flex items-start gap-3 text-sm text-destructive">
                ...
                <Card className="border-border shadow-card flex flex-col items-center justify-center py-16 gap-4">
                  ...
                  <div className="mt-2 space-y-1.5 text-xs text-muted-foreground">
                    {[
                      ...
                      'Near-match / fuzzy detection with lexical evidence',
                      'Semantic candidate discovery via Gemini (not counted as plagiarism)',
                      'Searches Crossref, OpenAlex & Unpaywall',
                      'PDF source text extraction',
                      'Inline passage highlighting with verified spans',
                    ].map(f => (
                ...
                <div className="space-y-4">
                  ...
                  {result.status === 'partial' && (
                    <div className="flex items-center gap-2 text-xs text-warning bg-warning/10 border border-warning/20 rounded-lg px-3 py-2">
                      ...
                      Partial results — one or more providers were unavailable. Results may be incomplete.
                    </div>
                  )}

                  {/* ── Scores (only when analysis ran) ── */}
                  {(result.status === 'completed' || result.status === 'partial') && (
                    <Card className={`border shadow-card ${scoreBorder(result.similarityScore)}`}>
                      <CardContent className="p-5">
                        <div className="flex items-center justify-between mb-3">
                          <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
                            <TextSearch className="w-4 h-4" /> Similarity Score
                          </span>
                        ...
                        <div className="flex items-end gap-3 mb-2">
                          ...
                          <span className="text-sm text-muted-foreground mb-1.5 font-medium uppercase tracking-wider">Similar</span>
                        </div>
                    ...
                    <CardContent className="p-0">
                      {result.sources.length === 0 ? (
                        <div className="p-5 text-center text-sm text-muted-foreground flex flex-col items-center gap-2">
                          ...
                          No verified sources found in checked databases.
                        </div>
                      ) : (
                            ...
                            <div key={i} className="p-4 hover:bg-muted/30 transition-colors">
                              <div className="flex justify-between items-start gap-2 mb-1.5">
                                <div className="flex-1 min-w-0">
                                  <p className="font-medium text-xs text-navy truncate" title={source.title}>
                                    {source.title}
                                  </p>
