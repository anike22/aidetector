...
import {
  Wand2, Sparkles, RefreshCw, ChevronRight, Loader2, X, FileEdit, ShieldCheck, Lock, ArrowRight, UserPlus, LogIn, Sparkle, Save, Trash2, Check, History
} from 'lucide-react';
import {
...
export function SEOAssistantWorkspace({
  ...
  const currentCost = useMemo(() => {
    return calculateBloggerAnalysisCost(wordCount);
  }, [wordCount]);
  ...
  const executeAnalysisComputation = useCallback((text: string, kw: string, currentAiLinksActive: boolean, currentAiGrammarActive: boolean) => {
    ...
    if (isAnalyzed) {
      ...
      analysisDebounceRef.current = setTimeout(() => {
        if (documentVersionRef.current === currentVersion && val.trim()) {
          const res = executeAnalysisComputation(val, keyword, aiLinksActive, false);
          if (res) {
    ...
    if (isAnalyzed) {
      ...
      analysisDebounceRef.current = setTimeout(() => {
        if (documentVersionRef.current === currentVersion && content.trim()) {
          const res = executeAnalysisComputation(content, val, aiLinksActive, aiGrammarActive);
          if (res) {
            setLastAnalyzedHash(currentHash);
            setIsStale(false);
          }
        ...
        <div className="border-b border-border bg-card px-4 md:px-6 py-3 flex flex-col xl:flex-row xl:items-center justify-between gap-3">
          <div className="min-w-0 flex-1">
            <div className="flex items-center flex-wrap gap-2">
              <h2 className="text-sm md:text-base font-bold text-foreground">
                {isBloggerLanding ? 'Blog Post SEO & AI Analysis Workspace' : 'SEO Writing Assistant'}
              </h2>
              {isBloggerLanding && (
            ...
            <p className="text-xs text-muted-foreground text-pretty mt-0.5">
              {isBloggerLanding 
                ? 'Type or paste your blog post below for instant SEO, readability, AI risk & plagiarism insights.' 
          ...
          <div className="flex items-center gap-2 flex-wrap shrink-0 justify-start xl:justify-end">
            <UsageBadge
              ...
              label={isBloggerLanding ? `Analyze Article · ${currentCost} Credits` : 'Daily AI uses'}
              operationCost={currentCost}
            />
