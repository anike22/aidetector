...
function isHtmlContent(content: string): boolean {
  ...
  return /^\s*<(?:h[1-6]|p|div|ul|ol|li|blockquote|pre|table|article|section|header|footer|main|aside|nav|br|img|a|strong|em|b|i|span|code|hr|figure|figcaption)\b[^>]*>/i.test(trimmed);
}
...
function markdownToHtml(content: string): string {
  ...
  html = html.replace(/((?:\|[^\n]+\|\r?\n)+)/g, (tableBlock) => {
    ...
    let tableHtml = '<table class="border-collapse border border-border my-6 w-full"><tbody>';
    let isHeader = true;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      cells.forEach((cell) => {
        if (isHeader && i === 0) {
          tableHtml += `<th class="border border-border p-2.5 bg-muted font-bold text-left">${cell}</th>`;
        } else {
          tableHtml += `<td class="border border-border p-2.5 text-left">${cell}</td>`;
        }
      });
      tableHtml += '</tr>';
    }
...
function enrichContentHtml(post: BlogPost) {
  ...
  // HTML content is published as-is with sanitization
  if (isHtmlContent(content)) {
    return sanitizeHtml(content);
  }

  // Markdown-aware formatting with sanitization
