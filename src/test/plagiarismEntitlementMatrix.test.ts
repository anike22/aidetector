...
describe('Plagiarism Entitlement and Billing Matrix Tests', () => {
  it('1. verifies rate table configuration for plagiarism_check and aliases', () => {
    ...
    expect(configCheck?.baseCreditCost).toBe(10);
    ...
    expect(configChecker?.baseCreditCost).toBe(10);
    expect(configChecker?.billingUnit).toBe('words_1000');
  });

  it('2. calculates accurate credit costs per 1,000 words for paid users', () => {
    // 500 words = 1 unit of 1000 = 10 credits
    expect(calculateOperationCreditCost('plagiarism_check', { words: 500 })).toBe(10);
    // 1000 words = 1 unit = 10 credits
    expect(calculateOperationCreditCost('plagiarism_check', { words: 1000 })).toBe(10);
    // 1001 words = 2 units = 20 credits
    expect(calculateOperationCreditCost('plagiarism_check', { words: 1001 })).toBe(20);
    // 2500 words = 3 units = 30 credits
    expect(calculateOperationCreditCost('plagiarism_check', { words: 2500 })).toBe(30);
    expect(calculateOperationCreditCost('plagiarism_checker', { words: 2500 })).toBe(30);
  });
