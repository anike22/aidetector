...
describe('Plagiarism Checker Guest Session & Error-State Regression', () => {
  ...
  it('5. Live DB check: guest session reserve with unique guest ID succeeds and settles atomically', async () => {
    const freshGuest = `v_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
    await supabase.rpc('issue_or_validate_guest_session', {
    ...
    if (res.error_code === 'GUEST_SESSION_RATE_LIMITED') {
      // Runner IP rate limited by PostgreSQL security policy
      expect(res.allowed).toBe(false);
      expect(res.reason).toContain('Too many new guest sessions');
      return;
    }
