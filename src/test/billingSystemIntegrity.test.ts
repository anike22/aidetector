...
describe('AIDetector.cx Billing System Repair & Integrity Audit', () => {
  ...
  describe('2. Subfeature Separation & Independent Pricing', () => {
    it('preserves independent pricing for existing billable subfeatures', () => {
      // Humanizer: 10 credits per 1,000 words
      expect(calculateOperationCreditCost('humanizer', { words: 1000 })).toBe(10);
      expect(calculateOperationCreditCost('humanizer', { words: 1200 })).toBe(20);

      // Generate Article / With AI: 5 credits fixed
      expect(calculateOperationCreditCost('generate_article', {})).toBe(5);

      // Plagiarism: 10 credits per 1,000 words
      expect(calculateOperationCreditCost('plagiarism_deep', { words: 1000 })).toBe(10);
      ...
      // Citation verification: 5 credits per 5 references
      expect(calculateOperationCreditCost('citation_verify', { references: 5 })).toBe(5);
      expect(calculateOperationCreditCost('citation_verify', { references: 10 })).toBe(10);
