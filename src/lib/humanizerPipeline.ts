...
export function extractProtectedEntities(text: string, userLockedTerms: string[] = []): ProtectedEntity[] {
  ...
  const numberRegex = /(\b\$?[0-9]+(?:,[0-9]{3})*(?:\.[0-9]+)?(?:\s*(?:%|percent|USD|EUR|GBP|km\/h|mph|kg|lbs|MB|GB|TB|ms|sec|min|hrs|days|years|st|nd|rd|th|k|M|B|°C|°F))?(?=\b|\s|[.,;!?]|$)|[0-9]+(?:\.[0-9]+)?°[CF])/gi;
...
export function validateRewriteIntegrity(
  ...
  const rewrTrimmed = rewrittenText.trim();
  const origWords = origTrimmed.split(/\s+/).filter(Boolean).length;
  const rewrWords = rewrTrimmed.split(/\s+/).filter(Boolean).length;
  const origParagraphs = origTrimmed.split(/\n\s*\n/).filter(p => p.trim().length > 0).length;
  const rewrParagraphs = rewrTrimmed.split(/\n\s*\n/).filter(p => p.trim().length > 0).length;

  if (origTrimmed.length > 0 && rewrTrimmed.length === 0) {
    omissionIssues.push('Output is empty — generation failed or was completely truncated.');
    warningFlags.push('Empty output detected');
  } else if ((origWords >= 30 && rewrWords < origWords * 0.45) || (origParagraphs >= 3 && rewrParagraphs <= 1) || (origParagraphs >= 2 && rewrParagraphs === 1 && origWords >= 60)) {
    ...
    if (terminalPunctuation.test(origTrimmed) && !terminalPunctuation.test(rewrTrimmed) && rewrWords > 15) {
      omissionIssues.push('Output ends abruptly without terminal punctuation — possible cutoff during generation.');
      warningFlags.push('Abrupt cutoff detected');
    }
  }
