...
export function matchInternalLinksToArticle(
  ...
  for (const link of candidateLinks) {
    ...
    if (isExisting && matchStart >= 0) {
      ...
      // Requirement 5: Analyse paragraph or the entire content to get the appropriate location for link anchor
      let bestParagraphIdx = 0;
      let bestParagraphScore = -1;

      for (let pIdx = 0; pIdx < paragraphOffsets.length; pIdx++) {
        const p = paragraphOffsets[pIdx];
        const pLower = p.text.toLowerCase();
        let pScore = 0;
        for (const tok of destTokens) {
          if (pLower.includes(tok)) pScore += 2;
        }
        if (primaryKeyword && pLower.includes(primaryKeyword.toLowerCase())) {
          pScore += 3;
        }
        if (pScore > bestParagraphScore) {
          bestParagraphScore = pScore;
          bestParagraphIdx = pIdx;
        }
      }
      ...
      for (let sIdx = 0; sIdx < sentenceOffsets.length; sIdx++) {
        ...
        let sScore = 0;
        for (const tok of destTokens) {
          if (sLower.includes(tok)) sScore += 2;
        }
        if (primaryKeyword && sLower.includes(primaryKeyword.toLowerCase())) {
          sScore += 3;
        }
        // Give priority bonus if the sentence is located within the best-matching paragraph
        const targetSentenceOffset = s.start;
        const matchingPara = paragraphOffsets[bestParagraphIdx];
        if (matchingPara && targetSentenceOffset >= matchingPara.start && targetSentenceOffset <= matchingPara.end + 4) {
          sScore += 2;
        }
