import importedBinaryPackage from './models/authorship_classifier_v1.json';
import importedPackage from './models/authorship_classifier_v2.json';

export const CLASSIFIER_VERSION = (importedPackage as { version?: string }).version ?? 'unknown';
...
async function loadPackage(): Promise<ModelPackage> {
  ...
  // In Deno, read the JSON file at runtime because JSON import assertions are not
  // supported by the Supabase Edge Function bundler. In Vite, use the static import.
  const deno = (globalThis as unknown as { Deno?: { readTextFile: (path: string) => Promise<string> } }).Deno;
  if (deno) {
    const url = new URL('./models/authorship_classifier_v2.json', import.meta.url);
    const raw = await deno.readTextFile(url.pathname);
    PKG = JSON.parse(raw) as ModelPackage;
  } else {
    PKG = importedPackage as unknown as ModelPackage;
  }
  return PKG;
}
...
async function loadBinaryPackage(): Promise<{
  ...
  const deno = (globalThis as unknown as { Deno?: { readTextFile: (path: string) => Promise<string> } }).Deno;
  if (deno) {
    const url = new URL('./models/authorship_classifier_v1.json', import.meta.url);
    const raw = await deno.readTextFile(url.pathname);
    BINARY_PKG = JSON.parse(raw) as {
      version: string;
      type: string;
      languages: string[];
      languageModels: Record<string, BinaryLanguageModel>;
    ...
    BINARY_PKG = importedBinaryPackage as unknown as {
      version: string;
      type: string;
      languages: string[];
      languageModels: Record<string, BinaryLanguageModel>;
