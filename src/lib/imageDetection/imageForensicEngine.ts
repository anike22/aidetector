 ...
 */
export function evaluateAIGenerationEvidence(
  ...
  if (detectedGenerators.length > 0 || provenance.digitalSourceType?.includes('trainedAlgorithmicMedia')) {
  ...
  } else if (provenance.watermark.detected && provenance.watermark.provider) {
    rawScore = Math.max(86, Math.min(97, 84 + (provenance.watermark.confidence || 90) * 0.12));
    reasoning.push(`Identified embedded synthetic watermark from ${provenance.watermark.provider}.`);
  } else if (provenance.cameraExif?.make && provenance.c2paStatus === 'Valid trusted credentials') {
    // Hardware cryptographically signed camera original (e.g. Leica CAI, Nikon authenticated hardware)
    rawScore = Math.min(8, Math.max(2, 3 + forensics.spectralArtifactScore * 0.04));
    reasoning.push('Valid camera hardware origin certificate and consistent sensor noise confirm authentic capture.');
  } else if (provenance.cameraExif?.make && !provenance.claimGenerator && !provenance.watermark.detected) {
    // Standard hardware camera EXIF headers present (Canon, Nikon, Sony, Apple iPhone, Samsung Galaxy, Google Pixel, etc.)
    const make = provenance.cameraExif.make;
    const model = provenance.cameraExif.model || `${make} Device`;
    // Authentic camera noise and optical characteristics yield low baseline probability (2-14%)
    rawScore = Math.min(14, Math.max(2, Math.round(4 + forensics.spectralArtifactScore * 0.12)));
    reasoning.push(`Original hardware camera EXIF headers detected (${make} ${model}) with optical lens capture characteristics.`);
    reasoning.push('Sensor noise distribution and frequency spectrum correspond to authentic optical camera capture rather than synthetic generation.');
  } else {
    // Multi-signal calibration based on forensic pixel residual analysis (e.g. stripped metadata or web uploads)
    ...
    // In authentic camera photographs where metadata was stripped by social media or messaging platforms:
    // Absence of AI markers and presence of natural optical noise profile produces a truthful, low score (4-22%).
    if (!provenance.claimGenerator && !provenance.watermark.detected && forensics.spectralArtifactScore < 40) {
      rawScore = Math.min(22, Math.max(3, Math.round(computedForensic * 0.65)));
      reasoning.push('Natural luminance gradient transitions and optical sensor noise profile detected without AI diffusion signatures.');
      if (provenance.exifIntegrity === 'Stripped / Cleaned metadata') {
        reasoning.push('Standard camera metadata has been stripped (common for messaging apps, screenshots, and web uploads), but underlying pixel residuals align with natural optical photography.');
      }
    } else {
